...
syn cluster shIfList	contains=@shLoopList,shDblBrace,shDblParen,shFunctionKey,shFunctionOne,shFunctionTwo,Nothing
syn cluster shLoopList	contains=@shCaseList,@shErrorList,shCaseEsac,shConditional,shDblBrace,shExpr,shFor,shIf,shOption,shSet,shTest,shTestOpr,shTouch,Nothing
if exists("b:is_kornshell") || exists("b:is_bash")
...
if !exists("g:sh_no_error")
 syn match   shDoError "\<done\>" contains=Conceal,Nothing
 ...
 syn match   shOK	'\.\(done\|fi\|in\|esac\)' contains=Conceal,Nothing
 if exists("b:is_kornshell") || exists("b:is_bash")
...
" ======
ShFoldIfDoFor syn region shDo	transparent	matchgroup=shConditional start="\<do\>" matchgroup=shConditional end="\v(do)@<=$"			contains=ALL
ShFoldIfDoFor syn region shIf	transparent	matchgroup=shConditional start="\<if\_s" matchgroup=shConditional skip=+-fi\>+ end="\<;\_s*then\>" end="\<fi\>"	contains=@shIfList,Nothing,Conceal
ShFoldIfDoFor syn region shFor		matchgroup=shLoop start="\<for\ze\_s\s*\%(((\)\@!" end="\<in\>" end="\<do\>"me=e-2			contains=@shLoopList,shDblParen,Nothing,Conceal skipwhite nextgroup=shCurlyIn
if exists("b:is_kornshell") || exists("b:is_bash")
...
ShFoldIfDoFor syn region	shCaseEsac	matchgroup=shConditional start="\<case\>" end="\<esac\>"	contains=@shCaseEsacList,Conceal,Nonthing
...
syntax keyword Keyword endif     conceal cchar=ℰ
syntax keyword Keyword else     conceal cchar=𝘌
syntax keyword Keyword case     conceal cchar=𝕔
syntax keyword Keyword fi       conceal cchar=|
syntax keyword Keyword esac     conceal cchar=|
syntax keyword Keyword done     conceal cchar=|

syntax cluster shEchoList       add=Nothing
syntax cluster shExprList2      add=Nothing
syntax cluster shSubShList      add=Nothing
syntax cluster shCaseList       add=Nothing
syntax cluster shCommandSubList add=Nothing
syntax cluster Spell            add=Nothing
syntax cluster shIdList         add=Nothing
syntax cluster shFunctionList   add=Nothing
syntax cluster shNoZSList       add=Nothing
syntax cluster shDerefList      add=Nothing
syntax cluster shCommandSubList add=Nothing
