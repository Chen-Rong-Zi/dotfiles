...
" =======================
if &buftype != 'nofile'
 syn region vimInsert               matchgroup=vimCommand start="^[: \t]*\(\d\+\(,\d\+\)\=\)\=a\%[ppend]$"                    matchgroup=vimCommand end="^\.$"" contains=Conceal
 syn region vimInsert               matchgroup=vimCommand start="^[: \t]*\(\d\+\(,\d\+\)\=\)\=c\%[hange]$"                    matchgroup=vimCommand end="^\.$"" contains=Conceal
 syn region vimInsert               matchgroup=vimCommand start="^[: \t]*\(\d\+\(,\d\+\)\=\)\=i\%[nsert]$"                    matchgroup=vimCommand end="^\.$"" contains=Conceal
endif
...
syn region vimKeymap matchgroup=vimCommand start="\<loadk\%[eymap]\>" end="\%$" contains=vimKeymapStart,Conceal
...
syn region        vimCommentString  contained oneline start='\S\s\+"'ms=e                 end='"' contains=Conceal
...
syn region        vimSet                              matchgroup=vimCommand start="\<\%(setl\%[ocal]\|setg\%[lobal]\|se\%[t]\)\>" skip="\%(\\\\\)*\\.\n\@!" end="$" end="|" matchgroup=vimNotation end="<[cC][rR]>" keepend contains=vimSetEqual,vimOption,vimErrSetting,vimComment,vim9Comment,vimSetString,vimSetMod,Conceal
...
syn region        vimVarList        contained         start="\[" end="]" contains=vimVar,vimContinue,Conceal
...
syn region        vimUnletVars      contained         start="$\I\|\h" skip="\n\s*\\" end="$" end="|" contains=vimVar,vimEnvvar,vimContinue,vimString,vimNumber,Conceal
...
" ================
syn region        vimEcho           oneline excludenl matchgroup=vimCommand start="\<ec\%[ho]\>" skip="\(\\\\\)*\\|" end="$\||" contains=vimFunc,vimFuncVar,vimString,vimVar,Conceal
syn region        vimExecute        oneline excludenl matchgroup=vimCommand start="\<exe\%[cute]\>" skip="\(\\\\\)*\\|" end="$\||\|<[cC][rR]>" contains=vimFuncVar,vimIsCommand,vimOper,vimNotation,vimOperParen,vimString,vimVar,Conceal
...
syn match vimFunc                   /\v<\h{-}\ze\(/                            contains=vimFuncEcho,vimFuncName,vimUserFunc,vimExecute
" syn match vimUserFunc contained     "\%(\%([sSgGbBwWtTlL]:\|<[sS][iI][dD]>\)\=\%(\w\+\.\)*\I[a-zA-Z0-9_.]*\)\|\<\u[a-zA-Z0-9.]*\>\|\<if\>"      contains=vimNotation
...
syn region        vimClusterName    contained         matchgroup=vimGroupName start="\h\w*" skip="\\\\\|\\|" matchgroup=vimSep end="$\||" contains=vimGroupAdd,vimGroupRem,vimSynContains,vimSynError,Conceal
...
syn region        vimSynKeyRegion   contained oneline keepend           matchgroup=vimGroupName start="\h\w*" skip="\\\\\|\\|" matchgroup=vimSep end="|\|$" contains=@vimSynKeyGroup,Conceal
...
syn region        vimSynMatchRegion contained keepend matchgroup=vimGroupName start="\h\w*" matchgroup=vimSep end="|\|$" contains=@vimSynMtchGroup,Conceal
...
syn region        vimSynRegion      contained keepend matchgroup=vimGroupName start="\h\w*" skip="\\\\\|\\|" end="|\|$" contains=@vimSynRegGroup,Conceal
...
syn region        vimSynRegPat      contained extend  start="\z([-`~!@#$%^&*_=+;:'",./?]\)"  skip="\\\\\|\\\z1"  end="\z1"  contains=@vimSynRegPatGroup,Conceal skipwhite nextgroup=vimSynPatMod,vimSynReg
...
syn region        vimHiKeyList      contained oneline start="\i\+" skip="\\\\\|\\|" end="$\||"              contains=@vimHiCluster,Conceal
...
syn region        vimHiLink         contained oneline matchgroup=vimCommand start="\(\<hi\%[ghlight]\s\+\)\@<=\(\(def\%[ault]\s\+\)\=link\>\|\<def\>\)" end="$"   contains=@vimHiCluster,Conceal
...
syn region        vimString         start="^\s*\\\z(['"]\)" skip='\\\\\|\\\z1' end="\z1" oneline keepend contains=@vimStringGroup,vimContinue,Conceal
...
if (g:vimsyn_embed =~# 'p' && has("perl")) && filereadable(s:perlpath)
 ...
 VimFoldp syn region vimPerlRegion  matchgroup=vimScriptDelim start=+pe\%[rl]\s*<<\s*\z(\S*\)\ze\(\s*["#].*\)\=$+ end=+^\z1\ze\(\s*[#"].*\)\=$+ contains=@vimPerlScript,Conceal
 VimFoldp syn region vimPerlRegion  matchgroup=vimScriptDelim start=+pe\%[rl]\s*<<\s*$+ end=+\.$+                                               contains=@vimPerlScript,Conceal
 syn cluster vimFuncBodyList        add=vimPerlRegion
else
...
if g:vimsyn_embed =~# 'P' && has("pythonx") && filereadable(s:pythonpath)
 ...
 VimFoldP syn region vimPythonRegion matchgroup=vimScriptDelim start=+py\%[thon][3x]\=\s*<<\s*\%(trim\s*\)\=\z(\S*\)\ze\(\s*#.*\)\=$+ end=+^\z1\ze\(\s*".*\)\=$+  contains=@vimPythonScript,Conceal
 VimFoldP syn region vimPythonRegion matchgroup=vimScriptDelim start=+py\%[thon][3x]\=\s*<<\s*\%(trim\s*\)\=$+ end=+\.$+                                                            contains=@vimPythonScript,Conceal
 VimFoldP syn region vimPythonRegion matchgroup=vimScriptDelim start=+Py\%[thon]2or3\s*<<\s*\%(trim\s*\)\=\z(\S*\)\ze\(\s*#.*\)\=$+ end=+^\z1\ze\(\s*".*\)\=$+    contains=@vimPythonScript,Conceal
 VimFoldP syn region vimPythonRegion matchgroup=vimScriptDelim start=+Py\%[thon]2or3\=\s*<<\s*\%(trim\s*\)\=$+ end=+\.$+                                                            contains=@vimPythonScript,Conceal
 syn cluster vimFuncBodyList        add=vimPythonRegion
else
...
if s:trytcl
 ...
 if (g:vimsyn_embed =~# 't' && has("tcl")) && filereadable(s:tclpath)
  ...
  VimFoldt syn region vimTclRegion matchgroup=vimScriptDelim start=+tc[l]\=\s*<<\s*\z(.*\)$+ end=+^\z1$+    contains=@vimTclScript,Conceal
  VimFoldt syn region vimTclRegion matchgroup=vimScriptDelim start=+tc[l]\=\s*<<\s*$+ end=+\.$+             contains=@vimTclScript,Conceal
...
if (g:vimsyn_embed =~# 'm' && has("mzscheme")) && filereadable(s:mzschemepath)
 ...
 let &isk= s:iskKeep
 unlet s:iskKeep
 VimFoldm syn region vimMzSchemeRegion matchgroup=vimScriptDelim start=+mz\%[scheme]\s*<<\s*\z(.*\)$+ end=+^\z1$+             contains=@vimMzSchemeScript,Conceal
 VimFoldm syn region vimMzSchemeRegion matchgroup=vimScriptDelim start=+mz\%[scheme]\s*<<\s*$+ end=+\.$+                      contains=@vimMzSchemeScript,Conceal
 syn cluster vimFuncBodyList        add=vimMzSchemeRegion
else
...
hi argument_variable ctermfg=yellow
hi link Conceal         Keyword
hi link script_variable Float
hi link global_variable Typedef
hi link window_variable Number
hi link buffer_variable Error
...
syntax match   Conceal           /\v<end%(for|if|def|function|while|class|try)>/     conceal    cchar=|
...
syntax region inBracket2 matchgroup=Bracket2 start=/(/ end=/)/ display contains=inBracket3,@hidden contained oneline
syntax region inBracket3 matchgroup=Bracket3 start=/(/ end=/)/ display contains=inBracket4,@hidden contained oneline
syntax region inBracket4 matchgroup=Bracket4 start=/(/ end=/)/ display contains=inBracket1,@hidden contained oneline

" syntax region inBracket1 matchgroup=Bracket1 start=/(/ end=/)/ display contains=ALLBUT,inBracket3,inBracket4,inBracket1 oneline
" syntax region inBracket2 matchgroup=Bracket2 start=/(/ end=/)/ display contains=ALLBUT,inBracket4,inBracket1,inBracket2 contained oneline
" syntax region inBracket3 matchgroup=Bracket3 start=/(/ end=/)/ display contains=ALLBUT,inBracket3,inBracket1,inBracket2 contained oneline
" syntax region inBracket4 matchgroup=Bracket4 start=/(/ end=/)/ display contains=ALLBUT,inBracket3,inBracket4,inBracket2 contained oneline
