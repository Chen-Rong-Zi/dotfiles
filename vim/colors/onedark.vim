...
call s:h("Conceal",      {   "fg": s:purple})  "placeholder   characters substituted for  concealed text   (see      'conceallevel')
...
call s:h("Normal",       {   "fg": s:foreground })  "normal         text
...
call s:h("SpecialKey",       { "fg": s:gutter_fg_grey })     "Meta         and      special     keys        listed    with    ":map",      also   for        text  used     to            show unprintable  characters in         the  text, 'listchars'. Generally: text       that         is      displayed  differently from what it really is.
...
call s:h("TabLine",          { "fg": s:comment_grey,  "bg":  s:black       })       "tab        pages       line,     not     active       tab    page       label
call s:h("TabLineFill",      { "fg": s:green          })     "tab          pages    line,       where       there     are     no           labels
call s:h("TabLineSel",       { "fg": s:green,         "bg":  s:black       })       "tab        pages       line,     active  tab          page   label
" call s:h("Terminal",         {     "fg": s:white,         "bg":  s:black       })       "terminal   window      (see      terminal-size-color)
call s:h("Terminal",         {     "fg": s:white,  })       "terminal   window      (see      terminal-size-color)
